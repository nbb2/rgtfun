...
The closest existing open-source tool to RGTFun is MagnumPI, which focuses on computing scattering angles, cross sections, and collision integrals for several (but not all) intermolecular potentials. In contrast, RGTFun extends the workflow beyond this to compute macroscopic transport coefficients (e.g., viscosity and diffusion) as well as simplified collision model parameters for the variable hard sphere (VHS) and variable soft sphere (VSS) models. RGTfun aims to provide an end-to-end calculation with a helpful graphical user interface (GUI). Furthermore, RGTfun shows the user the results of intermediate calculations (such as the distance of closest approach) so the user can verify that the algorithm is behaving correctly at each step. 
